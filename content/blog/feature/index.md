...
However, explaining these problems is not why I'm writing this post -- and honestly, I'm pretty uninformed about the other problems...
Except for problem 1, which I'm uninformed about as well, but enough hands-on practice to transmit 1% of cool knowledge and facts about it. 

Problem 1 is the problem of **interpretability**.
...
- Since the ImageNet class "Egyptian cat" has an index of 285, we know that in the fully connected last layer of the model (`fc`), neuron 285 is the greatest among the neurons in that layer (which are 1000 in total). This is because of the last operation (`argmax`), returns the index of the previous layer's neuron with the greatest activation.
...
Before our step function, we need to define what transforms we'll apply to our image before passing it to our model.
...
Now for a final technique, I'll introduce a sort-of obvious technique to get rid of noise which can work surprisingly well: simply applying gaussian blur to the image. Of course, if we do this repeatedly, we will only get a blurry image. If done ocasionally though, we can obtain good results.
...
While I was originally implementing transformational robustness, I misunderstood the concept and *actually transformed the visualization*, instead of *just doing gradient propagation on* the transformed image. During these trying times, I experimented with biasing the transformations to always increase the scale of the image. The result is an eternal zoom-in effect, but the atmosphere is that of endlessly submerging yourself on alien worlds.
...
I'd dare to say that these moving visualizations give us a different perspective to understand what a feature is representing.
...
- ResNet18 seems to have a bias towards the color green. This is a common feature among a lot of the models I've tested. *My theory*: since a lot of the classes and thus training data are animal-related, grass is a common denominator in the background of these pictures. Therefore, the model sees a lot of green during its training, and generates a bias.
